...
interface AsphyxiaAnalysisResult {
...
const AsphyxiaAnalyzer = () => {
  const navigate = useNavigate();
  const [video, setVideo] = useState<string | null>(null);
  const [frames, setFrames] = useState<string[]>([]);
  const [analyzing, setAnalyzing] = useState(false);
  const [result, setResult] = useState<AsphyxiaAnalysisResult | null>(null);
  ...
  const runAnalysis = async () => {
    ...
    try {
      const visualPrompt = `Analyze these ${frames.length} temporal frames of a newborn for visual signs of Birth Asphyxia.
      Look for:
      - Cyanosis (blue or pale discoloration of skin/lips)
      - Lack of movement or limpness (hypotonia)
      - Respiratory effort (chest retractions or gasping)
      
      Provide your visual findings in JSON format:
      {
        "visual_findings": "Detailed description of baby's color, tone, and visible effort",
      ...
      const visualResponse = await fetch(config.host, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "You are a specialized Neonatal Health AI. Analyze visual frames for Birth Asphyxia risk." },
            {
              role: "user",
              content: [
      ...
      const finalPrompt = `You are performing a final Birth Asphyxia assessment.
      
      VISUAL FINDINGS FROM EARLIER STEP:
      ${visualFindings.visual_findings}
      Visual Severity Estimate: ${visualFindings.severity_visual}
      
      Now, analyze the accompanying audio for cry patterns (weak, absent, or high-pitched).
      ...
      Provide the final analysis in the following JSON format:
      {
        ...
        "recommendations": ["immediate clinical actions"]
      }
      Only return the JSON.`;
      ...
      const finalResponse = await fetch(config.host, {
        ...
        body: JSON.stringify({
          model: config.model_name,
          messages: [
            { role: "system", content: "You are a specialized Neonatal Health AI. Integrate visual and audio data for Asphyxia diagnosis." },
            {
              role: "user",
              content: [
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 sm:p-8">
      ...
      <header className="max-w-6xl mx-auto flex items-center justify-between mb-12">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black tracking-tight text-slate-800">Asphyxia<span className="text-rose-600">Analyzer</span></h1>
        </div>
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
          ...
          <div className="relative group">
            <div 
              ...
              ${video ? 'border-transparent bg-white shadow-xl' : 
                isDragging ? 'border-rose-600 bg-rose-50 scale-[1.02]' : 'border-slate-200 bg-white hover:border-rose-500/50 hover:bg-slate-50'}
            `}>
              {video ? (
                ...
                >
                  <div className="w-16 h-16 bg-rose-50 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <Video className="w-8 h-8 text-rose-600" />
                  </div>
                  <h3 className="text-xl font-black mb-2 text-slate-800">Upload Birth Media</h3>
                  <p className="text-slate-500 text-sm max-w-[240px] font-medium">
                    Upload a video or photo of the newborn. We'll analyze visual and audio cues.
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-500 font-medium">
                  {processingVideo ? 'Extracting visual & audio signatures...' : 
                   video ? (
                     hasAudio === true ? 'Visual & Audio data ready.' : 
                     hasAudio === false ? 'Visual data ready (No audio track found).' :
                     'Ready for multi-modal analysis.'
                   ) : 'Waiting for upload...'}
                </p>
              </div>
          ...
          >
            {analyzing ? (
              <>
                ...
                Analyzing Multi-Modal Cues...
              </>
            ) : (
              <>
                <Search className="w-6 h-6" />
                Identify Asphyxia Risk
              </>
            )}
          </button>
        ...
        <section className="space-y-6">
            ...
            {result ? (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-6">
                  <div className={`p-2 rounded-full ${result.severity === 'none' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                    {result.severity === 'none' ? <CheckCircle2 className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
                  </div>
                </div>

                <div className="mb-8">
                  <span className="text-xs font-black tracking-widest text-rose-600 uppercase mb-2 block">Detection Result</span>
                  <h2 className="text-4xl font-black text-slate-800 mb-4">
                    {result.severity.charAt(0).toUpperCase() + result.severity.slice(1)} Risk
                  </h2>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${(result.confidence || 0.85) * 100}%` }} className={`h-full ${result.severity === 'none' ? 'bg-emerald-500' : 'bg-rose-600'}`} />
                    </div>
                ...
                <div className="space-y-6">
                  <div className="p-5 bg-slate-50 border border-slate-100 rounded-2xl">
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed italic border-l-2 border-rose-200 pl-4 mb-4 font-medium">
                      "{result.thinking}"
                    </p>
                    <div className="grid grid-cols-1 gap-4">
                      <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                        <p className="text-[10px] uppercase font-black text-slate-400 mb-1">Visual Cues</p>
                        <p className="text-xs text-slate-600 font-bold">{result.visual_observations}</p>
                      </div>
                      <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                        <p className="text-[10px] uppercase font-black text-slate-400 mb-1">Audio Cues</p>
              ...
              <div className="h-full min-h-[500px] bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center text-center p-8 text-slate-400">
                {!analyzing && (
                  <>
                    ...
                    <p className="max-w-[280px] font-medium">Upload a video to extract a representative visual frame and audio signatures for analysis.</p>
                  </>
                )}
                {analyzing && (
          ...
          <div className="p-6 bg-rose-50 border border-rose-100 rounded-2xl text-center">
            <p className="text-[10px] text-rose-600 leading-relaxed uppercase tracking-widest font-black">
              Critical Warning: Asphyxia is a time-sensitive emergency. This tool is for clinical assistance and validation, not a substitute for immediate neonatal resuscitation.
            </p>
          </div>
        </section>
      </main>
...
export default AsphyxiaAnalyzer;
