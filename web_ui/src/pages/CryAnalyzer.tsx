...
import { Upload, Video, Music, Search, RefreshCcw, Settings, AlertCircle, CheckCircle2, Info, Volume2, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';

interface CryAnalysisResult {
  cry_detected: boolean;
  type: "hunger" | "pain" | "discomfort" | "tired" | "none";
  confidence: number;
  thinking: string;
...
const CryAnalyzer = () => {
  ...
  const processFile = async (file: File) => {
    if (file && file.type.startsWith('video/')) {
      ...
      try {
        ...
        setError("Failed to extract media signatures.");
      } finally {
  ...
  const runAnalysis = async () => {
    ...
    try {
      const visualPrompt = `Analyze these ${frames.length} temporal frames of a crying baby for facial and physical cues.
      Look for:
      - Facial tension (brow furrowing, eye squeezing)
      - Mouth shape (openness, rigidity)
      - Skin color (redness, paleness)
      - Body movement (clenched fists, arching back)
      
      Provide your visual findings in JSON format:
      {
        "visual_findings": "Detailed description of facial and body language signatures",
      ...
      const visualResponse = await fetch(config.host, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: "You are a specialized Neonatal Health AI. Analyze facial frames for cry context." },
            {
              role: "user",
              content: [
      ...
      const finalPrompt = `You are performing a final baby cry analysis.
      
      VISUAL FINDINGS FROM EARLIER STEP:
      ${visualFindings.visual_findings}
      
      Now, analyze the accompanying audio for acoustic patterns (pitch, intensity, rhythm, pauses).
      Combine the visual findings with the acoustic data to determine the likely cause (hunger, pain, discomfort, etc.).
      ...
      Provide the final analysis in the following JSON format:
      {
        ...
        "thinking": "Step-by-step reasoning integrating facial cues and audio patterns",
        "visual_observations": "Summarized visual data",
        "audio_observations": "Analysis of cry patterns",
        "recommendations": ["list of actions"]
      }
      Only return the JSON.`;

      if (!audioBase64) {
        throw new Error("No audio track found in the video. Please upload a video with sound.");
      }
      ...
      const finalResponse = await fetch(config.host, {
        ...
        body: JSON.stringify({
          model: config.model_name,
          messages: [
            { role: "system", content: "You are a specialized Neonatal Health AI. Integrate facial and audio data for cry analysis." },
            {
              role: "user",
              content: [
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 sm:p-8">
      <header className="max-w-6xl mx-auto flex items-center justify-between mb-12">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-black tracking-tight text-slate-800">Cry<span className="text-purple-600">Analyzer</span></h1>
        </div>
        <div className="flex gap-4">
          <button 
            onClick={() => navigate('/')}
            className="px-4 py-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-sm font-bold text-slate-600 shadow-sm"
          >
            Jaundice
          </button>
          <button 
            onClick={() => navigate('/asphyxia-analyzer')}
            className="px-4 py-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-sm font-bold text-slate-600 shadow-sm"
          >
            Asphyxia
          </button>
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
          ...
          <div className="relative group">
            <div 
              ...
              ${video ? 'border-transparent bg-white shadow-xl' : 
                isDragging ? 'border-purple-600 bg-purple-50 scale-[1.02]' : 'border-slate-200 bg-white hover:border-purple-500/50 hover:bg-slate-50'}
            `}>
              {video ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-black mb-2 text-slate-800">Upload Cry Video</h3>
                  <p className="text-slate-500 text-sm max-w-[240px] font-medium">
                    Drag and drop a video clip of your baby crying. We'll analyze the audio patterns.
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              <h4 className="font-bold text-slate-800">
                {extractingAudio ? 'Extracting Audio...' : 
                 ...
                 hasAudio === false ? 'No Audio Detected' : 'Audio Extraction'}
              </h4>
              <p className="text-sm text-slate-500 font-medium">
                {extractingAudio ? 'Reading acoustic signatures...' :
                 hasAudio === true ? 'Audio patterns are ready for analysis.' :
                 hasAudio === false ? 'This video seems to be silent.' : 'Waiting for video upload...'}
              </p>
            </div>
        ...
        <section className="space-y-6">
          <AnimatePresence mode="wait">
            {result ? (
              <motion.div
                ...
                className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden"
              >
                <div className="absolute top-0 right-0 p-6">
                  <div className={`p-2 rounded-full ${result.confidence > 0.8 ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                    <CheckCircle2 className="w-6 h-6" />
                  </div>
                </div>

                <div className="mb-8">
                  <span className="text-xs font-black tracking-widest text-purple-600 uppercase mb-2 block">Detection Result</span>
                  <h2 className="text-4xl font-black text-slate-800 mb-4">
                    {result.cry_detected ? `${result.type.charAt(0).toUpperCase() + result.type.slice(1)} Cry` : 'No Cry Detected'}
                  </h2>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <motion.div 
                        ...
                        className={`h-full ${result.confidence > 0.8 ? 'bg-emerald-500' : 'bg-amber-500'}`}
                      />
                    </div>
                    <span className="text-sm font-bold text-slate-400">{Math.round(result.confidence * 100)}% Match</span>
                  </div>
                </div>

                <div className="space-y-6">
                  <div className="p-5 bg-slate-50 border border-slate-100 rounded-2xl">
                    <h3 className="text-sm font-black text-purple-600 uppercase tracking-widest mb-3 flex items-center gap-2">
                      <Search className="w-4 h-4" />
                      Acoustic Reasoning
                    </h3>
                    <p className="text-sm text-slate-600 leading-relaxed italic border-l-2 border-purple-200 pl-4 font-medium">
                      "{result.thinking}"
                    </p>
                  </div>
              ...
              <div className="h-full min-h-[400px] bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center text-center p-8 text-slate-400">
                {!analyzing && (
                  <>
                    <Volume2 className="w-12 h-12 mb-4 text-slate-200" />
                    <h3 className="text-xl font-black mb-2 text-slate-500">No Analysis Yet</h3>
                    <p className="max-w-[280px] font-medium">Upload a video and we'll analyze the audio spectrum for baby cry patterns.</p>
                  </>
                )}
                {analyzing && (
          ...
          <div className="p-6 bg-purple-50 border border-purple-100 rounded-2xl text-center">
            <p className="text-[10px] text-purple-600 font-black leading-relaxed uppercase tracking-widest">
              Note: This tool analyzes frequency and temporal patterns in audio. It is for informational purposes and should not replace clinical monitoring.
            </p>
          </div>
        </section>
      </main>
